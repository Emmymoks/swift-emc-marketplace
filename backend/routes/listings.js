...
router.post('/', async (req, res) => {
  try {
    ...
    const listing = new Listing({ owner: user._id, title, type, category, images, description, price, currency: currency || user.currency, location, approved: false });
    await listing.save();
    return res.json({ ok: true, listing });
  } catch (err) { res.status(500).json({ error: 'Server error' }); }
});

// get public listings (approved only). Supports query for category, type, search, and premium first
router.get('/', async (req, res) => {
  try {
    ...
    // premium first (simple)
    const listings = await Listing.find(filter).sort({ premiumUntil: -1, createdAt: -1 }).limit(200).populate('owner','username profilePhotoUrl location');
    res.json({ listings });
  } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
